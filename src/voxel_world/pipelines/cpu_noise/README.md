...
*   **入力**: 対象チャンク周辺3x3の `TerrainColumn` 高度・バイオーム成果物。
*   **依存関係**: 隣接チャンクのロード状態には依存しません。対象チャンクが周辺のFeature originを評価し、自領域に入る変更だけを収集します。
*   **処理**:
    *   チャンク境界からFeature最大幅だけ広げた地表を反復処理します。
    *   バイオームごとのフィーチャー生成確率をチェックします。
    *   `BiomeRegistry` で定義されたフィーチャー（例: 木）を配置します。
*   **出力**: 対象チャンク内だけにクリップされたボクセル変更のリスト (`Vec<(IVec3, Voxel)>`)。
*   **アクション**: 共通のボクセル変更APIを使用して `ChunkMap` に変更を適用します。
